document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (ruleName === 'email' && value.trim() !== '' && !validators.email(value)) {
          showError(field, getErrorMessage(field, 'email'));
          return false;
        }

        if (ruleName === 'minLength' && value.trim() !== '' && !validators.minLength(value, ruleValue)) {
          showError(field, getErrorMessage(field, 'minLength', ruleValue));
          return false;
        }

        if (ruleName === 'phone' && value.trim() !== '' && !validators.phone(value)) {
  ...
  const animatedSelector = '.animate-on-scroll, [data-animate]';
  ...
  function initScrollAnimations() {
    ...
    if ('IntersectionObserver' in window) {
      ...
      elements.forEach(function (el) {
        el.classList.add(animationClass, visibleClass);
      });
    }
  }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '2bc27f3e-5a8e-4d19-8cdb-a142d43d273d',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '2bc27f3e-5a8e-4d19-8cdb-a142d43d273d';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '2bc27f3e-5a8e-4d19-8cdb-a142d43d273d';
